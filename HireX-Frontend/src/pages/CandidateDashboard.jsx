        ...
        const load = async () => {
            try {
                ...
                const [res1, res2, res3] = await Promise.all([
                    ...
                    fetch('http://127.0.0.1:8000/jobs/', { headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) } })
                ]);
                if (res1.ok) {
                ...
                if (res3.ok) {
                    ...
                    // Filter active jobs that the user hasn't applied to? Or just show all.
                    // For now show all active jobs.
                    if (mounted) setJobs(j || []);
                }
            } catch (e) {
                ...
                */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                    {jobs.length === 0 ? (
                        ...
                        jobs.map(job => (
                            <div key={job.id} className="bg-gray-50 rounded-xl p-8 flex flex-col items-center text-center pb-6">
                                <div className="w-full text-left mb-4">
                                    <h3 className="text-lg font-bold text-gray-900">{job.title}</h3>
                                    <p className="text-gray-700 text-sm">Skills: {job.skills ? job.skills.map(s => s.name).join(', ') : 'N/A'}</p>
                                    <p className="text-gray-600 text-xs mt-3 leading-relaxed line-clamp-2">
                                        {job.description}
                                    </p>
                                    {job.deadline && <p className="text-xs text-red-500 mt-2">Deadline: {new Date(job.deadline).toLocaleDateString()}</p>}
                                </div>
                                <div className="w-full border-t border-gray-300 my-4"></div>
                                <button
                                    onClick={() => onNavigate(`application-details`, { jobId: job.id })}
